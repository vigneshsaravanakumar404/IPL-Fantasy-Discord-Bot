...
class UtilityGroup(app_commands.Group):
    ...
    async def ping(self, interaction):
        ...
        await interaction.client.get_guild(Constants.IPL_FANTASY_SERVER).get_channel(Constants.LOGS_CHANNEL).send(embed=embed)
    ...
    async def refresh(self, interaction):
        ...
        if time_difference.total_seconds() < 60 * 60 * 24:
            embed = discord.Embed(title=f'{discord.utils.escape_markdown(interaction.user.display_name)} Used /refresh',
                                  color=discord.Color.red())
            embed.set_author(name=interaction.user.display_name, icon_url=interaction.user.avatar.url)
            embed.add_field(name="Data Refresh Failed", value="Data was refreshed less than 5 minutes ago!", inline=False)

            await interaction.response.send_message(
                f"{interaction.user.mention} Data was refreshed less than 5 minutes ago!", ephemeral=True)
            await interaction.client.get_guild(Constants.IPL_FANTASY_SERVER).get_channel(Constants.LOGS_CHANNEL).send(embed=embed)
        else:
            ...
            embed = discord.Embed(title=f'{discord.utils.escape_markdown(interaction.user.display_name)} Used /refresh',
                                  color=discord.Color.gold())
            embed.set_author(name=interaction.user.display_name, icon_url=interaction.user.avatar.url)
            embed.add_field(name="Data Refreshed", value="Data has been refreshed successfully!", inline=False)

            await interaction.response.send_message(
                f"{interaction.user.mention} Data has been refreshed successfully!", ephemeral=True)
            await interaction.client.get_guild(Constants.IPL_FANTASY_SERVER).get_channel(Constants.LOGS_CHANNEL).send(embed=embed)

async def setup(client):
    client.tree.add_command(UtilityGroup(name="utility", description="utlity and admin commands"))